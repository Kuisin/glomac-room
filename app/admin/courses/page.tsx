...
const checkValid = (data: CsvData) => {
    ...
    if (!data.endOfRepeat || data.endOfRepeat === '') message.push('End of Repeat');
...
const repeatSchedules = (data: CsvData[]) => {
    ...
    data.forEach(row => {
        ...
        const endOfRepeat = new Date(row.endOfRepeat);

        for (let date = new Date(startDate); date <= endOfRepeat; date = addDays(date, 7)) {
...
export default function CourseReservations() {
    ...
    const addData = () => {
        setSelectedData({ index: csvData.length, title: '', startTime: '', endTime: '', room: '', type: '', user: '', day: '', endOfRepeat: '', selected: false });
        setPopup(true);
    }
    ...
    return (
        <div id="app" className="flex flex-col h-screen bg-white text-black">
            <header className="sticky top-0 left-0 z-40 w-full flex items-center justify-between bg-white px-4 py-4 shadow">
                <h1 className="font-bold">Forest Gateway 空き教室（繰り返し予定の登録）</h1>
                <button
            ...
            <div className='p-4'>
                <div className='mb-4'>
                    ...
                    >Required Fields: Title, Type, Start Time, End Time, Room (Optional: User, Description)</p>
                </div>
                {headerMapDialog && (
